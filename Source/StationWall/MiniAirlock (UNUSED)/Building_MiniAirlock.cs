...
namespace DecompressionMod
{
    public class Building_MiniAirlock : Building, ISelectable
    {
        ...
        public override bool IsAirtight
        {
            get
            {
                if (!fullyInitialized || !Spawned)
                    return true; // Treat as sealed until ready

                return !(innerDoorOpen && outerDoorOpen);
            }
        }
        ...
        public override bool ExchangeVacuum
        {
            get
            {
                // Before fully initialized or not spawned yet → never exchange
                if (!fullyInitialized || !Spawned)
                    return false;

                // During cycling, maintain current airtight state
                if (isCycling)
                    return false;
        ...
        {
            // CRITICAL: Set state AND fully initialized BEFORE calling base.SpawnSetup
            if (!respawningAfterLoad)
            {
                ...
                fullyInitialized = true; // MOVED: Set this BEFORE base.SpawnSetup
                autoCycleCooldown = 0;
            }
            else
            {
            ...
            // Now call base spawn setup - vacuum system will see correct state
            base.SpawnSetup(map, respawningAfterLoad);

            // Force region rebuild to ensure proper room splitting
            // Do this after base spawn so the building is fully registered
            map.regionDirtyer.DirtyCells.Add(Position);
        ...
        private void NotifyVacuumSystemChanged()
        {
            if (!Spawned || Map == null) return;

            // Force region rebuild around the airlock
            Map.regionDirtyer.DirtyCells.Add(Position);

            // Also dirty adjacent cells to ensure proper room connections
            foreach (IntVec3 adj in GenAdj.CellsAdjacentCardinal(this))
            {
                if (adj.InBounds(Map))
                {
        ...
        private void CompleteCycle()
        {
            ...
            isCycling = false;
            cycleCompleted = true;
            autoCycleCooldown = AutoCycleCooldownTicks;
        ...
        // Add this debug method to help troubleshoot (remove in final version)
        public override string GetInspectString()
        {
            ...
            sb.AppendLine($"Fully Initialized: {fullyInitialized}");
            sb.AppendLine($"Spawned: {Spawned}");
            sb.AppendLine($"IsAirtight: {IsAirtight}");
            sb.AppendLine($"ExchangeVacuum: {ExchangeVacuum}");
            ...
            // DEBUG: Show room information
            if (Spawned && Map != null)
            {
                var room = Position.GetRoom(Map);
                if (room != null)
                {
                    sb.AppendLine($"Room ID: {room.ID}, Vacuum: {room.Vacuum:F2}");
                }
                else
                {
